...
func PullAllControlProposed(c *gin.Context) {
	...
	for _, control := range controls {
		...
		for _, relevance := range relevances {
			...
			currentValue, err := strconv.ParseFloat(strings.TrimSuffix(impl.PercentProposed, "%"), 64)
			if err != nil {
				continue
			}
	...
	if len(dataToAdd) > 0 {
		if err := saveResultsPropused(engine.(*xorm.Engine), dataToAdd); err != nil {
			c.Set("Response", err.Error())
			c.Status(http.StatusInternalServerError)
			return
		}
	...
	if len(dataToUpdate) > 0 {
		if err := saveResultsPropused(engine.(*xorm.Engine), dataToUpdate); err != nil {
			c.Set("Response", err.Error())
			c.Status(http.StatusInternalServerError)
			return
		}
